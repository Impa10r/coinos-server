...
async function* scanGetJSON(match: string, count = 1000) {
  ...
  async function* drain() {
    ...
    for (let i = 0; i < vals.length; i++) {
      ...
      if (!raw) continue;
      const v = String(raw);
      if (v[0] !== "{") continue; // skip mapping strings / nulls
      try {
  ...
  for await (const keys of db.scanIterator({ MATCH: match, COUNT: count })) {
...
for await (const [, u] of scanGetJSON("user:*")) {
...
for await (const keys of db.scanIterator({ MATCH: "balance:*", COUNT: 1000 })) {
  for (const key of keys as unknown as string[]) {
    const uid = key.slice("balance:".length);
...
let scanned = 0;
const t0 = Date.now();
for await (const [, p] of scanGetJSON("payment:*", 2000)) {
  scanned++;
