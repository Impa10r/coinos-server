...
## `@asoltys+clightning-client+0.2.0.patch`

Fixes the reconnect logic in `@asoltys/clightning-client` (v0.2.0) so a dropped
cl RPC connection actually recovers. (Originally written against v0.1.10; the
bug and the surrounding code are identical in 0.2.0, so the patch was retargeted
— verified with `git apply --check` against the installed 0.2.0 package.)
...
`git apply patches/@asoltys+clightning-client+0.2.0.patch` against the installed
package.
